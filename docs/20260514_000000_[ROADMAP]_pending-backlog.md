...
- Document: 20260514*000000*[ROADMAP]\_pending-backlog.md
- Category: ROADMAP

## Purpose

ISSUE-094 — moved from root `PENDING.md` into the canonical docs
location so the backlog is discoverable, tracked, and follows the
project's documentation naming convention. The content below is the
verbatim free-form dump from the original file. Each bullet is a
candidate GitHub issue waiting on the maintainer's curation; the
backlog is intentionally not pre-shaped because the items vary too
widely in scope to bucket programmatically.

## How to use this file

1. Scan for the next item you want to act on.
2. Translate it into a GitHub issue (one issue per discrete idea).
3. Strike the item from this doc once the issue exists.
4. When the file empties, delete it.
...
→ Ink <DiffView> component renders each hunk with: - Red background for removals - Green background for additions - Cyan accent for file headers - Interactive [Accept] [Keep Mine] [Accept All] buttons per file
...
Renders per file: - Header: "File 3 of 7: rules/security.md (+12 -3)" - DiffViewer (colored hunks from buildUnifiedDiff) - Action bar: [A]ccept incoming [K]eep mine [S]kip Accept A[l]l Skip Al[l] - Summary footer: "4 accepted, 1 kept, 2 remaining"
...
├──────────────────────────────┼──────────────────────────────────────┼──────────────────────────────────────┤
│ HUB*ACTIONS array │ src/cli/hub.ts:47 │ GroupedMenu consumes this │
├──────────────────────────────┼──────────────────────────────────────┼──────────────────────────────────────┤
│ PROJECT*\* constants │ src/constants.ts │ Banner/theme consume these │
├──────────────────────────────┼──────────────────────────────────────┼──────────────────────────────────────┤
