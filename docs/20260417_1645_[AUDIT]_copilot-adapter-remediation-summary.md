...
- [ ] Update codi-dev-agent-creator.prompt.md: Add "Enumerate all supported formats" to Step 3 (Write Process)
...
- [ ] Create `docs/adapter-specification-checklist-template.md` with format:
  ...
  ```
- [ ] Add to codi-dev-agent-creator.prompt.md Step 1: "Verify all official platform formats in specification checklist"
...
- [ ] Read codi-dev-agent-creator.prompt.md (9-step lifecycle)
...
mcp__graph - code__query_code_graph("What functions call sanitizeNameForPath? What depends on it?");

// Step 2: Get code snippets to understand current patterns
mcp__graph -
  code__get_code_snippet(
...
7. Use `/codi-dev-compare-preset` to see what's local vs upstream
