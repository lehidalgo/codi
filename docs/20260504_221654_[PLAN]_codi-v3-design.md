...
1. `codi-dev-team-charter` — Iron Laws inject en SessionStart
...
Sin tabla separada. Una skill = `codi-dev-rule-feedback` (captura) + `codi-dev-refine-rules` (cribado).
...
- Endpoints `/v1/notes/*`.
- Parser wikilinks.
- Skills builtin: `codi-recall`, `codi-remember`, `codi-dev-rule-feedback`, `codi-dev-refine-rules`, `codi-session-log`.
