...
| Linting/formatting | pre-commit | per-language | ESLint, Prettier, ruff, gofmt, etc. |
| Security analysis | pre-commit | `security_scan` | bandit (Python), gosec (Go), brakeman (Ruby), phpcs-security (PHP) |
| Baseline drift check | pre-push | always enabled | Detect template content drift before push |
