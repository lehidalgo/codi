...
- Skills only generate into agents that support them (currently: Claude Code, Codex, Windsurf, Cline).
...
- No "new version available" banner appears when running Codi commands. You need to check npm manually or set up a dependency update tool (Dependabot, Renovate).
