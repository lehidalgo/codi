...
- **Conflict-marker validator no longer flags documentation examples** — the v2.14.0 `E_CONFLICT_MARKERS` check (in `codi validate` and the `codi-conflict-marker-check.mjs` pre-commit hook) was matching markers anywhere in artifact content, including legitimate documentation in the `codi-dev-operations` skill that teaches manual merge resolution. As a result, `codi init` on any project that installed `dev-operations` (default in `codi-dev` and `codi-power-user` presets) would scaffold `.codi/` correctly but silently fail to generate `CLAUDE.md` / `.cursorrules` / `AGENTS.md` and silently skip pre-commit hook installation, surfacing only as `"generated": false` / `"hooksInstalled": false` in the JSON output with no human-readable error. The scanner now skips markers inside fenced code blocks (` ``` ` and `~~~`) and inside `<example>` tag regions. The same logic is mirrored into the inlined pre-commit hook script.
