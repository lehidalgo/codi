...
- **Unselecting a coding agent during `codi init --customize` now removes its directories** — orphan detection deleted the generated files but left empty husks like `.cursor/`, `.cursor/rules/`, `.cursor/skills/` behind. New `pruneEmptyAdapterDirs` helper walks parents of every deleted file plus the unselected agent's declared root paths (`configRoot`, `rules`, `skills`, `agents`), `fs.rmdir`-ing each one deepest-first. Non-empty dirs fail harmlessly via `ENOTEMPTY` so any user files inside are always preserved.
- **Destructive operations now snapshot to a recoverable backup** — `codi init`, `codi init --customize`, `codi update`, `codi clean` (non-`--all`), `codi preset install`, and the "Add from external source" wizard all open a backup before mutating files and write the manifest only on success. The backup captures source (`.codi/`), output (state-tracked agent files), and pre-existing files (e.g. a hand-written `CLAUDE.md` not yet tracked by codi). Crash mid-operation leaves no partial backup once `pruneIncompleteBackups` runs at the start of the next snapshot.
...
- **Backup lifecycle (`openBackup` -> `handle.append` -> `handle.finalise`)** in `src/core/backup/backup-manager.ts` replaces the single-shot `createBackup`. Callers receive a `BackupHandle` and can append additional paths mid-operation (e.g. orphans discovered during reconciliation). The manifest is written LAST as a commit marker, so a crash before `finalise()` leaves a partial dir that the next `openBackup` sweeps via `pruneIncompleteBackups`. New types in `src/core/backup/types.ts`: `BackupTrigger`, `BackupScope`, `RetentionStrategy`, `SnapshotOptions`, `BackupManifestV2`, `BackupManifestEntry`, `BackupHandle`, `OpenBackupError`, `OpenBackupResult`. The legacy `createBackup(projectRoot, configDir)` is preserved as a thin wrapper over `openBackup -> finalise` for backwards compatibility with `generate.ts`.
- **Manifest v2 with v1 compat shim** in `src/core/backup/backup-manifest.ts`. v2 manifests record `version`, `timestamp`, `trigger`, `codiVersion`, and a per-file `BackupManifestEntry[]` with `scope` (`source` | `output`) plus optional `preExisting` / `deleted` flags. Legacy v1 manifests (`{timestamp, files: string[]}`) are upgraded on read with `scope: "output"` and `trigger: "generate"`.
- **Retention helpers** in `src/core/backup/backup-retention.ts`: `listSealedBackups` (filters out partial dirs), `pruneIncompleteBackups` (sweeps any backup without a manifest), `evictOldest` (FIFO eviction), and `interactiveEvict` (`@clack/prompts` TUI with double-confirm; cancellation aborts the destructive op via the `RETENTION_CANCELLED_ERROR` constant). Backup cap raised from 5 to 50 (`MAX_BACKUPS`).
- **Source / pre-existing collectors** in `src/core/backup/backup-collectors.ts`: `collectSourceFiles` walks `.codi/` recursively excluding the new `BACKUP_EXCLUDE_DIRS` constant (`.codi/backups`, `.codi/.session`, `.codi/feedback`); `collectPreExistingFiles` probes every `ALL_ADAPTERS` target path and returns files present on disk but not yet in `state.agents` (e.g. a hand-written `CLAUDE.md` from before `codi init`).
- **`pruneEmptyAdapterDirs`** in `src/core/generator/prune-empty-adapter-dirs.ts` — removes empty adapter root dirs (`.cursor/`, `.claude/`, etc.) after orphan deletion when an agent is unselected. Threaded into `applyConfiguration` alongside the new optional `BackupHandle` parameter; orphan paths are appended to the open backup with `scope: "output", deleted: true` BEFORE deletion so revert can restore them.
- **`connectBackup`** in `src/core/backup/backup-source.ts` — adapts a backup directory into the existing `ExternalSource` shape consumed by `runArtifactSelectionFromSource`, so `codi revert` reuses the import wizard for source-restore. Throws when the backup has no `.codi/` subtree or no standard artifact dirs (`rules/`, `skills/`, `agents/`, `mcp-servers/`); the revert handler then falls back to direct file restore.
- **`codi backup` command** (`--list`, `--delete <ts...>`, `--prune`) in `src/cli/backup.ts` — sibling to `codi revert` for backup management without restoring. Registered in `src/cli.ts` and `CLI_COMMANDS`.
- **`codi revert` overhaul** — interactive TUI selection when no flag is given, `--dry-run` flag that prints what would happen via the manifest's per-scope counts, automatic pre-revert snapshot (trigger `pre-revert`, captures source + output + pre-existing) so reverting is itself reversible, restore via `connectBackup` -> `runArtifactSelectionFromSource` for backups that contain `.codi/` source, and a direct `restoreBackup` fallback for legacy output-only backups. Preserves the `--list`, `--last`, and `--backup <ts>` flags.
- **New error codes** `E_BACKUP_CANCELLED`, `E_NO_BACKUPS`, `E_GENERAL` in `src/core/output/error-catalog.ts` for the new backup lifecycle.
- **`withBackup` helper** in `src/cli/backup-cli-helpers.ts` — wraps a destructive operation with the open / finalise / abort lifecycle, used by `update.ts` and (indirectly) `init.ts` via `applyConfigurationWithBackup` in `init-helpers.ts`.
