...
- **Unselecting an agent during `codi init --customize` now removes its directories** — orphan detection deleted the files but left empty husks (`.cursor/`, `.cursor/rules/`, `.claude/skills/`). The new `pruneEmptyAdapterDirs` step `fs.rmdir`s parents of deleted files and the removed agent's root dirs, deepest-first. Non-empty dirs fail harmlessly via `ENOTEMPTY` so user files inside are preserved.
