...
def build_executive_summary(doc: Document) -> None:
    ...
    add_callout(doc, "Core value proposition",
...
def build_problem(doc: Document) -> None:
    ...
    add_bullet(doc, "Security rules defined in one agent never propagate to others — gaps remain.", "Security gaps.")
    add_bullet(doc, "Code style and testing policies drift between tools — inconsistent enforcement.", "Policy drift.")
    add_bullet(doc, "Teams maintain 5 separate config files with duplicate content — wasted effort.", "Maintenance burden.")
    add_bullet(doc, "No audit trail — no record of who changed what and when.", "No visibility.")
    add_bullet(doc, "Onboarding new agents requires recreating configuration from scratch.", "Slow onboarding.")
...
def build_what_is_codi(doc: Document) -> None:
    ...
    add_bullet(doc, "All configuration lives in .codi/ — a single, version-controlled directory.", "One source of truth.")
    add_bullet(doc, "A change to .codi/ propagates to all agent config files via codi generate.", "Zero drift.")
    add_bullet(doc, "Teams choose a preset; individuals add personal overrides in ~/.codi/user.yaml.", "Team + individual balance.")
    add_bullet(doc, "Verification tokens detect when generated files are manually edited.", "Integrity verification.")
    add_bullet(doc, "Flags can be locked to prevent overrides — security_scan: locked: true.", "Policy enforcement.")
    add_bullet(doc, "Presets are shareable bundles — install from ZIP, GitHub, or registry.", "Reusable team configs.")
    add_heading(doc, "How It Works — Four Steps", level=2)
    add_table(doc,
        ["Step", "Command", "What Happens"],
        [
            ["1. Initialize", "codi init", "Interactive wizard creates .codi/ with chosen preset"],
            ["2. Customize", "codi add rule security", "Add rules, skills, agents to .codi/"],
            ["3. Generate", "codi generate", "Write CLAUDE.md, .cursorrules, AGENTS.md, etc."],
            ["4. Monitor", "codi status", "Detect drift between .codi/ and generated files"],
        ],
...
def build_core_concepts(doc: Document) -> None:
    ...
    add_table(doc,
        ["Rule", "Purpose"],
        [
            ["codi-security", "OWASP Top 10:2025, secret management, input validation, auth/authorization"],
            ["codi-testing", "Testing strategy (integration > unit), 80% coverage, TDD, contract testing"],
            ["codi-code-style", "Naming conventions, function size (<30 lines), immutability, no deep imports"],
            ["codi-typescript", "strict: true, satisfies operator, path aliases, async patterns, Zod validation"],
            ["codi-git-workflow", "Conventional commits, atomic commits, trunk-based dev, no force-push"],
            ["codi-architecture", "Hexagonal architecture, modular monolith, dependency injection, CQRS"],
            ["codi-performance", "Core Web Vitals, N+1 prevention, connection pooling, caching strategy"],
            ["codi-documentation", "Diataxis framework, JSDoc, ADRs, runbooks, file naming convention"],
            ["codi-error-handling", "Never swallow errors, Result types, circuit breakers, bulkhead isolation"],
            ["codi-production-mindset", "OpenTelemetry, SLOs, feature flags, blue-green deployments"],
        ],
        col_widths=[2.2, 4.3]
    )
    add_body(doc, "27 rule templates available. Each rule specifies: name, description, priority (high/medium/low), scope (file patterns), and managed_by (codi or user).", italic=True, color=RL3_GRAY)
    ...
    add_table(doc,
        ...
        [
            ["Code Quality", "codi-code-review, codi-security-scan, codi-test-coverage, codi-e2e-testing"],
            ["Git Workflow", "codi-commit (conventional commits, pre-commit checks, message validation)"],
            ["Codi Platform", "codi-preset-creator, codi-rule-creator, codi-skill-creator, codi-agent-creator"],
            ["Document Generation", "codi-doc-engine, codi-deck-engine, codi-documentation"],
            ["Design", "codi-frontend-design, codi-canvas-design, codi-rl3-brand"],
            ["Content", "codi-content-factory, codi-internal-comms, codi-codebase-onboarding"],
        ],
        col_widths=[2.0, 4.5]
    )
    add_body(doc, "44 skill templates available. Skills live in .codi/skills/{name}/SKILL.md and support subdirectories: scripts/, references/, assets/, evals/.", italic=True, color=RL3_GRAY)
    ...
    add_table(doc,
        ["Agent", "Purpose", "Tools"],
        [
            ["codi-code-reviewer", "Senior code reviewer. Severity matrix (CRITICAL/HIGH/MEDIUM/LOW). Confidence filtering >80%.", "Read, Grep, Glob, Bash"],
            ["codi-security-analyzer", "OWASP vulnerability scanner. Covers injection, auth, supply chain, API security.", "Read, Grep, Glob, Bash"],
            ["codi-test-generator", "Generates unit, integration, and e2e test suites from existing code.", "Read, Write, Grep, Glob, Bash"],
        ],
        col_widths=[1.8, 3.2, 1.5]
    )
    add_body(doc, "22 agent templates available. Agents are defined in .codi/agents/*.md.", italic=True, color=RL3_GRAY)
    ...
    add_table(doc,
        ["Command", "Purpose"],
        [
            ["/codi-commit", "Create a well-structured git commit following conventional commits format"],
            ["/codi-review", "Review recent code changes for quality, security, and best practices"],
            ["/codi-test-run", "Run the project test suite and report results with coverage"],
            ["/codi-security-scan", "Run a comprehensive security scan and report vulnerabilities"],
            ["/codi-test-coverage", "Measure test coverage and identify untested code paths"],
            ["/codi-refactor", "Identify and safely remove dead code and redundant abstractions"],
...
def build_flags(doc: Document) -> None:
    ...
    add_table(doc,
        ...
        [
            ["enforced", "Always active. Non-negotiable. Stops the resolution chain.", "No"],
            ...
            ["conditional", "Apply only if agent, file pattern, or language matches.", "Yes"],
        ],
    ...
    add_table(doc,
        ...
        [
            ["auto_commit", "boolean", "false", "Automatic commits after agent changes"],
            ["test_before_commit", "boolean", "true", "Pre-commit hook runs tests before allowing commit"],
            ["security_scan", "boolean", "true", "Mandatory secret detection scan on staged files"],
            ...
            ["allow_force_push", "boolean", "false", "Allow force push to remote branches"],
            ["require_pr_review", "boolean", "true", "PR review required before merge"],
            ["mcp_allowed_servers", "string[]", "[]", "Whitelist of allowed MCP server names"],
            ["require_documentation", "boolean", "false", "Documentation required for all new code"],
            ["allowed_languages", "string[]", "[*]", "Restrict which languages agents can use"],
            ["progressive_loading", "enum", "metadata", "Skill inlining strategy for Windsurf/Cline"],
            ["drift_detection", "enum", "warn", "How to report config drift: error / warn / off"],
            ["auto_generate_on_change", "boolean", "true", "Auto-regenerate config when .codi/ changes"],
        ],
        col_widths=[1.8, 0.9, 0.9, 2.9]
    )
    add_callout(doc, "Enforced flags in the recommended preset",
...
def build_presets(doc: Document) -> None:
    ...
    add_table(doc,
        ...
        [
            ["codi-minimal", "Rapid prototyping", "Security off, no test requirements, all actions permitted. Fast and flexible."],
            ["codi-balanced", "Most teams (default)", "Security on, strict type-checking, no force-push, PR review required."],
            ["codi-strict", "Regulated environments", "Security locked, tests required, file deletion restricted, no force-push."],
            ["codi-fullstack", "Web/app development", "Broad rule set: testing, security, TypeScript, architecture, documentation."],
            ["codi-dev", "Codi CLI development", "Strict TypeScript, anti-hardcoding, safe release workflow, full QA suite."],
            ["codi-power-user", "Advanced daily workflow", "Graph exploration, day tracking, error diagnosis, enhanced commit tools."],
        ],
    ...
    add_table(doc,
        ...
        [
            ["Create", "codi preset create my-setup", "Interactive wizard: choose flags, rules, skills, agents"],
            ["Export", "codi preset export my-setup", "Package as .zip for sharing"],
            ["Install (ZIP)", "codi preset install ./my-setup.zip", "Apply preset to current project"],
            ["Install (GitHub)", "codi preset install github:org/repo", "Install from public/private repository"],
            ["List", "codi preset list", "Show installed and available presets"],
            ["Update", "codi preset update", "Pull newer versions of managed artifacts"],
            ["Remove", "codi preset remove my-setup", "Clean up preset artifacts and state"],
        ],
    ...
    add_callout(doc, "Managed vs user artifacts",
