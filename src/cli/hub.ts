...
import { marketplaceSearchHandler, marketplaceInstallHandler } from './marketplace.js';
import {
...
import { addRuleHandler, addSkillHandler, addAgentHandler, addCommandHandler } from './add.js';
...
export const HUB_ACTIONS: HubAction[] = [
  // Setup & Config
  { value: 'init', label: 'Initialize project', hint: 'Set up .codi/ with agents, presets, and rules', requiresProject: false, group: 'setup' },
  { value: 'add', label: 'Add artifact', hint: 'Create a rule, skill, agent, or command', requiresProject: true, group: 'setup' },
  { value: 'generate', label: 'Generate configs', hint: 'Rebuild all agent configuration files', requiresProject: true, group: 'setup' },
  { value: 'preset', label: 'Manage presets', hint: 'List, create, install, or export presets', requiresProject: true, group: 'setup' },
  // Build & Share
  { value: 'skill-export', label: 'Export skill', hint: 'Package a skill for marketplace sharing', requiresProject: true, group: 'build' },
  { value: 'marketplace', label: 'Search marketplace', hint: 'Find and install community skills', requiresProject: true, group: 'build' },
  { value: 'contribute', label: 'Contribute to community', hint: 'Share your artifacts with the codi project', requiresProject: true, group: 'build' },
  // Monitor & Maintain
  { value: 'status', label: 'Project status', hint: 'Check drift and sync state', requiresProject: true, group: 'monitor' },
  { value: 'doctor', label: 'Health check', hint: 'Diagnose project issues', requiresProject: true, group: 'monitor' },
  { value: 'validate', label: 'Validate config', hint: 'Check .codi/ configuration is valid', requiresProject: true, group: 'monitor' },
  { value: 'verify', label: 'Verify agent awareness', hint: 'Check if agents loaded their configs', requiresProject: true, group: 'monitor' },
  { value: 'compliance', label: 'Compliance report', hint: 'Full check: doctor + status + verification', requiresProject: true, group: 'monitor' },
  { value: 'ci', label: 'CI checks', hint: 'Run all validation checks for CI pipelines', requiresProject: true, group: 'monitor' },
  { value: 'clean', label: 'Clean generated files', hint: 'Remove agent config files', requiresProject: true, group: 'monitor' },
  { value: 'update', label: 'Update templates', hint: 'Update rules, skills, and agents to latest', requiresProject: true, group: 'monitor' },
  { value: 'revert', label: 'Revert to backup', hint: 'Restore generated files from a previous backup', requiresProject: true, group: 'monitor' },
  { value: 'watch', label: 'Watch for changes', hint: 'Auto-regenerate on .codi/ changes (long-running)', requiresProject: true, group: 'monitor' },
  { value: 'docs-update', label: 'Update docs counts', hint: 'Sync documentation counts with templates', requiresProject: true, group: 'monitor' },
];
...
export async function runCommandCenter(projectRoot: string): Promise<void> {
  ...
  if (!hasProject) {
    p.log.warn('No .codi/ directory found. Initialize a project first or select "Initialize project".');
  }
  ...
  p.log.step(`Running: ${actions.find(a => a.value === selected)?.label ?? selected}`);
...
async function routeAction(action: string, projectRoot: string): Promise<void> {
  switch (action) {
    case 'init': await handleInit(projectRoot); break;
    case 'add': await handleAdd(projectRoot); break;
    case 'generate': await handleGenerate(projectRoot); break;
    case 'preset': await handlePresetMenu(projectRoot); break;
    case 'skill-export': await handleSkillExport(projectRoot); break;
    case 'marketplace': await handleMarketplace(projectRoot); break;
    case 'contribute': await handleContribute(projectRoot); break;
    case 'status': await printResult(statusHandler(projectRoot)); break;
    case 'doctor': await handleDoctor(projectRoot); break;
    case 'validate': await printResult(validateHandler(projectRoot)); break;
    case 'verify': await handleVerify(projectRoot); break;
    case 'compliance': await handleCompliance(projectRoot); break;
    case 'ci': await printResult(ciHandler(projectRoot)); break;
    case 'clean': await handleClean(projectRoot); break;
    case 'update': await handleUpdate(projectRoot); break;
    case 'revert': await handleRevert(projectRoot); break;
    case 'watch': showCliOnly('watch', 'codi watch'); break;
    case 'docs-update': await printResult(docsUpdateHandler(projectRoot)); break;
  }
}
...
async function printResult(promise: Promise<{ exitCode: number }>): Promise<void> {
  const result = await promise;
  process.stdout.write(formatHuman(result as never) + '\n');
}

function showCliOnly(command: string, usage: string): void {
  p.log.info(`"${command}" is a long-running process. Run it directly from the CLI:`);
  p.log.info(`  ${usage}`);
...
async function handleGenerate(projectRoot: string): Promise<void> {
  ...
  const mode = await p.select({
    ...
    options: [
      { label: 'Normal', value: 'normal' as const, hint: 'Write files to disk' },
      { label: 'Dry run', value: 'dry-run' as const, hint: 'Show what would be generated without writing' },
      { label: 'Force', value: 'force' as const, hint: 'Regenerate even if unchanged' },
    ],
  });
...
async function handleClean(projectRoot: string): Promise<void> {
  ...
  if (!dryRun) {
    const target = cleanAll ? '.codi/ and all generated files' : 'generated agent config files';
...
async function handleVerify(projectRoot: string): Promise<void> {
  const mode = await p.select({
    ...
    options: [
      { label: 'Show verification prompt', value: 'show', hint: 'Display the prompt to paste into your agent' },
      { label: 'Check agent response', value: 'check', hint: 'Validate a pasted agent response' },
    ],
  });
  if (isCancelled(mode)) return;
...
async function handleRevert(projectRoot: string): Promise<void> {
  const mode = await p.select({
    message: 'Revert mode',
    options: [
      { label: 'List available backups', value: 'list', hint: 'Show all backup timestamps' },
      { label: 'Restore most recent backup', value: 'last', hint: 'Quick restore to last state' },
      { label: 'Restore specific backup', value: 'specific', hint: 'Choose a backup by timestamp' },
    ],
  });
  if (isCancelled(mode)) return;
  ...
  switch (mode) {
    ...
    case 'last': {
      const confirmed = await p.confirm({
        message: 'Restore from most recent backup? This overwrites current generated files.',
      });
...
async function handleMarketplace(projectRoot: string): Promise<void> {
  ...
  if (result.success && result.data?.results && result.data.results.length > 0) {
    ...
    const installResult = await marketplaceInstallHandler(projectRoot, skillName, {});
...
async function handlePresetMenu(projectRoot: string): Promise<void> {
  const action = await p.select({
    ...
    options: [
      { label: 'List installed presets', value: 'list', hint: 'Show all available presets' },
      { label: 'Create new preset', value: 'create', hint: 'Interactive preset builder' },
      { label: 'Install from source', value: 'install', hint: 'ZIP, GitHub, or registry' },
      { label: 'Export preset', value: 'export', hint: 'Package as ZIP for sharing' },
      { label: 'Edit preset', value: 'edit', hint: 'Modify an installed preset' },
  ...
  switch (action) {
    ...
    case 'export': {
      ...
      const result = await presetExportHandler(projectRoot, name, format, output ?? '.');
      process.stdout.write(formatHuman(result) + '\n');
      break;
    }
    case 'edit': {
