...
export function registerAddCommand(program: Command): void {
  ...
  addCmd
    ...
    .action(
      async (name: string | undefined, cmdOptions: Record<string, unknown>) => {
      ...
      async (name: string | undefined, cmdOptions: Record<string, unknown>) => {
        ...
        if (options.all) {
          ...
          for (const tmpl of AVAILABLE_COMMAND_TEMPLATES) {
            const result = await addCommandHandler(process.cwd(), tmpl, {
              template: tmpl,
            });
            results.push({ name: tmpl, success: result.success });
          }
            ...
            errors: [
              {
                code: "E_CONFIG_INVALID",
                message:
                  "Command name required. Use --all to add all templates.",
                hint: "",
                severity: "error",
                context: {},
              },
  ...
  addCmd
    ...
    .action(
      async (
        ...
        _cmdOptions: Record<string, unknown>,
      ) => {
        const globalOptions = program.opts() as GlobalOptions;
        initFromOptions(globalOptions);

        if (!name && !globalOptions.json) {
          await handleWizardFlow(
            "brand",
            brandAsArtifactHandler,
            globalOptions,
          );
            ...
            errors: [
              {
                code: "E_CONFIG_INVALID",
                message:
                  "MCP server name required. Use --all to add all templates.",
                hint: "",
                severity: "error",
                context: {},
              },
