...
export type ArtifactType = 'rule' | 'skill' | 'agent' | 'command';
...
export async function selectArtifactType(): Promise<ArtifactType | null> {
  ...
  const type = await p.select({
    ...
    options: [
      { label: 'Rule', value: 'rule' as const, hint: 'Enforce a coding standard or convention' },
      { label: 'Skill', value: 'skill' as const, hint: 'Define a reusable workflow' },
      { label: 'Agent', value: 'agent' as const, hint: 'Create a specialized subagent' },
      { label: 'Command', value: 'command' as const, hint: 'Add a slash command' },
    ],
  });
...
export async function runAddWizard(type: ArtifactType): Promise<AddWizardResult | null> {
  ...
  const mode = await p.select({
    ...
    options: [
      { label: 'From templates', value: 'templates' as const, hint: `Choose from ${templates.length} available templates` },
      { label: 'Blank (custom)', value: 'blank' as const, hint: 'Start with an empty skeleton' },
    ],
  });
  ...
  const name = await p.text({
    ...
    validate: (value) => {
      ...
      if (!NAME_PATTERN_STRICT.test(value)) return 'Use lowercase letters, numbers, and hyphens (must start with letter)';
    },
  });
