...
description: Use when the user wants to restructure code without changing observable behavior — extract a module, deepen a shallow abstraction, decouple tightly-coupled modules, or improve test seams. Manages a structured workflow through intent, baseline, plan, execute, verify, done — with a mandatory baseline phase that captures current behavior before any change. Triggers on "refactor", "restructure", "decouple", "deepen", "improve architecture", "extract", "cleanup". Not for new features (use feature-workflow), bug fixes (use bug-fix-workflow), or schema migrations (use migration-workflow).
---
...
```bash
devloop run refactor "<one-line description>"
devloop run refactor --from-story US-NNN "<one-line>"
```
...
- No specific target yet (just "find opportunities") → invoke `devloop:architecture-review` first.
