---
name: sheets-sync
description: Use when manifest events need to project into the project Google Sheet — invoked programmatically by any devloop workflow that mutates project state. Reads Sheet config from .devloop/project.json and authenticates via the configured service account. Honors a column-zone discipline — planning columns are human-owned, execution columns are devloop-owned. Appends an Audit row on every write; queues events when the Sheet is unreachable. Manifest is the source of truth; the Sheet is a live mirror. Body documents the zone rules, the event-to-column mapping, and the elicitation rule for missing config.
...
If the Sheet is unreachable, the workflow does not stop — events queue in the manifest and a daemon flushes them later. If the Sheet diverges, `devloop sheets reconcile` rebuilds it from the manifest.
...
- `devloop sheets daemon` — tails the manifest, pushes queued events on Sheet recovery.
- `devloop sheets reconcile` — manual recovery; rebuilds from manifest.
...
| Execution | Devloop          | `workflow_type`, `branch`, `commit_shas`, `design_doc_path`, `pr_url`, `pr_state`, `merged_sha`, `merged_at`, `started_at`, `completed_at`, `status`                                         |
...
**Elicitation rule.** Missing config = ASK. If `.devloop/project.json` lacks `sheet_id`, surface the choice to the user. Never invent.
