...
name: {{name}}
description: |
  Use when a repository needs git-lifecycle quality gates (pre-commit,
  commit-msg, pre-push), when CI integration must be audited, when branch
  naming convention is missing, or when the user asks for industry-grade hook
  setup. Triggers on "set up pre-commit", "configure git hooks", "audit the
  quality gates", "add commit-msg validator", "enforce branch naming",
  "install gitleaks", "pre-push checks needed". Body documents the three
  modes and the 5-category Hook Contract. First-run repository setup
  (chained from \`init-knowledge-base\`). Periodic audit via
  \`/${PROJECT_NAME}:{{name}}\`. After-the-fact when a CI failure surfaces a
  gap that should have been caught locally.
...
Audit, configure, and verify git-lifecycle quality gates. Adapts to the host stack. Composes with ${PROJECT_NAME}'s agent-runtime hooks (separate layer).
