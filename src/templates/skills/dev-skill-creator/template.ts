...
description: |
  ...
  ${PROJECT_NAME}-dev-rule-creator), creating an agent (use
  ${PROJECT_NAME}-dev-agent-creator), packaging multiple artifacts as a
  preset (use ${PROJECT_NAME}-dev-preset-creator), or writing general
...
- You want to enforce **constraints** on all code — that is a **rule** (\`${PROJECT_NAME}-dev-rule-creator\`).
- You want the agent to perform a **task** end-to-end — that is a **skill** (this flow).
- You want to dispatch a **specialist worker** that produces a structured report — that is an **agent** (\`${PROJECT_NAME}-dev-agent-creator\`).
...
- User wants to create a rule — use ${PROJECT_NAME}-dev-rule-creator
- User wants to create an agent — use ${PROJECT_NAME}-dev-agent-creator
- User wants to bundle multiple artifacts as a preset — use ${PROJECT_NAME}-dev-preset-creator
- User wants to write plain project documentation — use ${PROJECT_NAME}-project-documentation
- User wants to review or refine from collected feedback — use ${PROJECT_NAME}-dev-refine-rules (REVIEW mode)
...
- **${PROJECT_NAME}-dev-agent-creator** — Create subagent definitions to bundle with a skill
- **${PROJECT_NAME}-dev-rule-feedback** — Collect rule observations encountered while creating skills
`;
