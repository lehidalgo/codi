...
import { runEval, findProjectRoot, type EvalItem, type EvalOutput } from "./run-eval.js";
...
async function runLoop(opts: {
  ...
  for (let iteration = 1; iteration <= opts.maxIterations; iteration++) {
    ...
    if (opts.liveReportPath) {
      ...
      writeFileSync(opts.liveReportPath, generateReport(partialOutput as never), "utf-8");
    }
...
if (isDirectExecution) {
  ...
  if (reportVal !== "none") {
    if (reportVal === "auto" || !reportVal) {
      ...
      liveReportPath = join(tmpdir(), `skill_description_report_${name}_${ts}.html`);
    } else {
  ...
  if (resultsDir && liveReportPath) {
    writeFileSync(join(resultsDir, "report.html"), generateReport(output as never), "utf-8");
