...
import { existsSync, statSync, readdirSync, writeFileSync, mkdirSync, unlinkSync } from "node:fs";
...
function shouldExclude(relPath: string, isRootLevel: boolean): boolean {
  ...
  if (isRootLevel && parts.length > 0 && ROOT_EXCLUDE_DIRS.has(parts[0]!)) return true;
...
function packageSkill(skillPath: string, outputDir?: string): string | null {
  ...
  try {
    ...
    console.error(`Error creating .skill file: ${err instanceof Error ? err.message : err}`);
    return null;
  }
}
...
if (isDirectExecution) {
  ...
  if (args.length < 1) {
    console.log("Usage: npx tsx package-skill.ts <skill-directory> [output-directory]");
    process.exit(1);
  }
