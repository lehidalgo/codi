...
export function scanForPromptInjection(content: string, filePath: string): SecurityFinding[] {
  return matchPatterns(content, filePath, INJECTION_PATTERNS, "prompt_injection", true);
}

export function scanForMaliciousScripts(content: string, filePath: string): SecurityFinding[] {
  return matchPatterns(content, filePath, SCRIPT_PATTERNS, "malicious_script", false);
}

export function scanForDataExfiltration(content: string, filePath: string): SecurityFinding[] {
  const findings = matchPatterns(content, filePath, EXFIL_PATTERNS, "data_exfiltration", false);
  findings.push(
    ...matchPatterns(content, filePath, DEPENDENCY_PATTERNS, "suspicious_dependency", false),
  );
  return findings;
}

export async function validateFileType(filePath: string): Promise<SecurityFinding | null> {
...
export function validateFrontmatter(content: string, filePath: string): SecurityFinding[] {
  const findings: SecurityFinding[] = [];
...
export async function runSecurityScan(skillDir: string): Promise<SecurityReport> {
  const findings: SecurityFinding[] = [];
  ...
  for (const file of files) {
    ...
    const codeExtensions = new Set([".py", ".sh", ".bash", ".js", ".ts", ".mjs", ".rb", ".pl"]);
