...
**Also write rule feedback:** If the error pattern was caused by a missing or incorrect rule, write a structured observation to \\\`${PROJECT_DIR}/feedback/rules/\\\` (see the rule-feedback skill for the JSON format). This ensures the pattern is captured for future \\\`/${PROJECT_NAME}-dev-refine-rules\\\` review even if the user doesn't create a rule now.
...
- **${PROJECT_NAME}-dev-rule-creator** — Create a preventive rule if the error pattern suggests a systemic fix
`;
