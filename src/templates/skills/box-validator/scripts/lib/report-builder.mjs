// Builds the final JSON report from annotated tree + violations. Produces
// the scored, summarised output the agent reads to decide iterate vs deliver.
//
// Scoring is complexity-aware: penalties are divided by a normalizer that
// grows with node count so that the same absolute number of minor issues
// is more forgiving on a large tree than a small one. See context.mjs for
// the scoreNormalizer formula.

export function buildReport({ root, siblingGroups, violations, threshold, context }) {
...
// Natural-language summary the agent reads to drive iteration. Groups
// violations by rule and names the worst-offending regions.
