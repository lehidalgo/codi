...
async function runAuthCheck(flags: CliFlags): Promise<void> {
  ...
  if (scopeProbe !== undefined) {
    if (scopeProbe.outcome === "ok") {
      ...
      console.log(`      bash <plugin-root>/skills/dev-sheets-sync/scripts/oauth-user-setup.sh`);
      console.log(`    On personal Gmail (unverified-app block):`);
      console.log(
        `      bash <plugin-root>/skills/dev-sheets-sync/scripts/oauth-user-project-client-setup.sh`,
      );
