...
description: |
  ...
  /${PROJECT_NAME}-dev-refine-rules or phrases like "show feedback summary",
  ...
  ${PROJECT_NAME}-dev-rule-creator), emitting new observations during work (that
  happens automatically via ${PROJECT_NAME}-dev-rule-feedback), or general quality
  audits without collected feedback (use ${PROJECT_NAME}-dev-compare-preset).
...
| **REFINE** | "refine rules", "improve rules", "process feedback", /${PROJECT_NAME}-dev-refine-rules | Interactive one-at-a-time approval + edits | Yes |

Start in REVIEW mode when the user just wants visibility. Move to REFINE when they are ready to apply changes.

## Skip When

- User wants to create a brand-new rule — use ${PROJECT_NAME}-dev-rule-creator
- User is emitting a new observation during coding work — ${PROJECT_NAME}-dev-rule-feedback handles it automatically
- User wants to diff local rules vs upstream templates — use ${PROJECT_NAME}-dev-compare-preset
- No feedback has been collected yet — wait for ${PROJECT_NAME}-dev-rule-feedback to accumulate observations
...
---
Run /${PROJECT_NAME}-dev-refine-rules to review these one by one and propose changes.
\\\`\\\`\\\`
...
- **${PROJECT_NAME}-dev-rule-feedback** — Collects the observations this skill reviews
- **${PROJECT_NAME}-dev-rule-creator** — Create entirely new rules (when observations suggest gaps)
- **${PROJECT_NAME}-dev-skill-creator** — Refine or create new skills
