...
description: |
  ...
  "run these in parallel". Replaces ad-hoc Task tool use and prevents
  context-pollution between coordinator and worker. Body documents the two
  modes and the implementer status protocol. Phase execute of any workflow
  when the plan has ≥3 discrete tasks, or when ≥2 unrelated investigations
  can run concurrently. Standalone via \`/${PROJECT_NAME}:{{name}}\` for
  ad-hoc fan-out or plan execution outside the workflow phases.
