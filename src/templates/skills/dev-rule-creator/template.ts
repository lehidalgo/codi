...
description: |
  ...
  creation, use ${PROJECT_NAME}-dev-artifact-contributor to share, package as
  ZIP, or open a PR with the new rule. Do NOT activate for creating a skill
  (use ${PROJECT_NAME}-dev-skill-creator), creating an agent (use
  ${PROJECT_NAME}-dev-agent-creator), or refining an existing rule from
  collected feedback (use ${PROJECT_NAME}-dev-refine-rules).
...
- **${PROJECT_NAME}-dev-rule-feedback** — Get feedback on an existing rule's quality and trigger precision
- **${PROJECT_NAME}-dev-refine-rules** — Batch-refine multiple rules for consistency and clarity
`;
