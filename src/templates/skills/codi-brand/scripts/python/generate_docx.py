#!/usr/bin/env python3
"""
generate_docx.py — Codi-branded DOCX generator (python-docx, FALLBACK runtime).
...
def _add_callout(doc: Document, text: str) -> None:
    ...
    _set_font(run, "primary", 12, italic=True)

    # Left border via XML (python-docx lacks native paragraph borders)
    ...
    # Dark shading matching Codi surface color
    shd = OxmlElement("w:shd")
    shd.set(qn("w:val"), "clear")
    shd.set(qn("w:fill"), bt.hex_color("surface").upper())
    pPr.append(shd)
...
def build_cover(doc: Document, content: dict) -> None:
    ...
    p = doc.add_paragraph()
    run = p.add_run("codi")
    _set_font(run, "primary", 18, font_key="pptx_headlines", bold=True)
    p.paragraph_format.space_after = Pt(20)

    # Title
    p = doc.add_paragraph()
    run = p.add_run(content["title"])
    _set_font(run, "text_primary", 28, font_key="pptx_headlines", bold=True)
    ...
    meta_parts.append(str(datetime.now().year))
    p = doc.add_paragraph()
    run = p.add_run(" · ".join(meta_parts))
    _set_font(run, "text_muted", 11)
...
def build_section(doc: Document, section: dict) -> None:
    ...
    for run in heading.runs:
        _set_font(run, "text_primary", 20, font_key="pptx_headlines", bold=True)
    ...
    for run in p.runs:
        _set_font(run, "text_secondary", 12)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Codi DOCX generator (python-docx fallback)"
    )
