...
After 3 failed hypotheses → STOP. The architecture is wrong, not the implementation. Hand off to \\\`${PROJECT_NAME}:architecture-review\\\`.
...
- \\\`condition-based-waiting.md\\\` — guidance on replacing arbitrary sleeps with condition-based waits.
- \\\`references/condition-based-waiting-example.ts\\\` — reference TypeScript implementation.
