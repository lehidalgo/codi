...
description: |
  ...
  on /${PROJECT_NAME}-dev-compare-preset. Produces a report of local improvements
  ...
  creating a new preset (use ${PROJECT_NAME}-dev-preset-creator), or opening the
  PR itself (use ${PROJECT_NAME}-dev-artifact-contributor).
...
- User runs \`/${PROJECT_NAME}-dev-compare-preset\`

## Skip When

- User wants to install a preset from a ZIP — use \\\`${PROJECT_CLI} preset install <path>\\\`
- User wants to create or package a new preset — use ${PROJECT_NAME}-dev-preset-creator
- User wants to open a PR with their improvements — use ${PROJECT_NAME}-dev-artifact-contributor (this skill only identifies candidates)
...
- **${PROJECT_NAME}-dev-preset-creator** — Create or package a preset before comparing it
- **${PROJECT_NAME}-dev-artifact-contributor** — Contribute local improvements found during comparison back upstream
`;
