...
The deepening recommendation produced by `architecture-review` becomes the input to `refactor-workflow`:

- Refactor `intent` phase — adopt the chosen deepening candidate as the structural friction
- Refactor `baseline` phase — capture current behavior across the modules being merged (the old shallow tests' assertions are the candidates; verify they assert behavior, not internal state)
- Refactor `plan` phase — write the deepened-interface design in the plan's "Module depth analysis" section, with the dependency category from this document determining the testing strategy
- Refactor `execute` phase — add the new shape (without removing the old), update callers, then remove the old shape; old shallow tests are deleted only after the new interface tests are green
