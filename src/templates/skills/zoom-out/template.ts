...
name: {{name}}
description: |
  Use when the user does not know an area of code well, when the agent has
  narrowed too tight on a single function or file, when starting a new feature
  in unfamiliar territory, or when a debugging session has lost the forest for
  the trees. Triggers on "zoom out", "give me the big picture", "I don't know
  this area", "step back". User-invoked only; does not auto-fire. Body
  documents the trigger phrase and the map output shape. Standalone via
  \`/${PROJECT_NAME}:{{name}}\`. Lightweight reset, not a workflow phase. Call
  before architecture-review if the user has not yet identified which area to
  survey.
...
- After zooming out, common follow-ups:
  - User wants to deepen a shallow cluster → invoke \\\`${PROJECT_NAME}:architecture-review\\\`
  - User wants to start a feature in this area → invoke \\\`${PROJECT_NAME}:feature-workflow\\\`
  - User wants to fix a specific bug surfaced during the map → invoke \\\`${PROJECT_NAME}:bug-fix-workflow\\\`
