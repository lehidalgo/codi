...
export const FLAG_CATALOG: Record<string, FlagSpec> = {
  ...
  disallow_push_to_protected: {
    ...
    default: false,
    hook: null,
    description: "Block direct push to protected branches (main/master/develop)",
    hint: "ADR-013 Paso 8 — adds Bash(git push ...) deny patterns to .claude/settings.json",
  },
  disallow_no_verify: {
    type: "boolean",
    default: false,
    hook: null,
    description: "Block git commit/push with --no-verify",
    hint: "Prevents bypassing pre-commit / pre-push hooks via --no-verify",
  },
  disallow_env_edits: {
    type: "boolean",
    default: false,
    hook: null,
    description: "Block Edit/Write on .env* files",
    hint: "Adds Edit/Write deny patterns for .env, .env.local, .env.production, etc.",
  },
  disallow_git_config_global: {
    type: "boolean",
    default: false,
    hook: null,
    description: "Block 'git config --global' invocations",
    hint: "Prevents the agent from silently changing global git config",
  },
  disallow_pip_install: {
    type: "boolean",
    default: false,
    hook: null,
    description: "Block 'pip install' invocations",
    hint: "Forces explicit user approval for Python dependency installs",
  },
  allow_feature_branch_workflow: {
    type: "boolean",
    default: false,
    hook: null,
    description: "Allow feature/bugfix/chore/release/hotfix branch operations",
    hint: "Adds allow patterns for force-push-with-lease and branch-create on feature-style branches",
  },
  require_pr_review: {
    type: "boolean",
    default: true,
