...
import { ARTIFACT_LAYOUT, ARTIFACT_TYPES, ARTIFACT_DIR_NAMES } from "../artifact-types.js";
...
export async function discoverArtifacts(
  ...
  for (const type of ARTIFACT_TYPES) {
    ...
    const typeRoot = path.join(rootPath, layout.dirName);
    const exists = await fs
      .stat(typeRoot)
      .then((s) => s.isDirectory())
      .catch(() => false);
    ...
    for (const entry of entries) {
      ...
      const relPath = path.join(layout.dirName, entry.name);
      if (layout.kind === "file") {
