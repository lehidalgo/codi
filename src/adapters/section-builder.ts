...
export function buildProjectOverview(config: NormalizedConfig): string {
  ...
  lines.push(`**Managed by:** [Codi](https://github.com/lehidalgo/codi)`);
...
export function buildArchitectureSummary(config: NormalizedConfig): string {
  ...
  if (config.skills.length > 0) {
    lines.push(`**Skills** (${config.skills.length}): ${config.skills.map(s => s.name).join(', ')}`);
  }
  if (config.agents.length > 0) {
    lines.push(`**Agents** (${config.agents.length}): ${config.agents.map(a => a.name).join(', ')}`);
  }
  if (config.commands.length > 0) {
    lines.push(`**Commands** (${config.commands.length}): ${config.commands.map(c => c.name).join(', ')}`);
  }
