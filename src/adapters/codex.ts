...
export const codexAdapter: AgentAdapter = {
  ...
  async generate(config: NormalizedConfig, _options: GenerateOptions): Promise<GeneratedFile[]> {
    ...
    {
      // TOML order matters: any root-level key (developer_instructions,
      // model, etc.) MUST come BEFORE any `[section]` header, otherwise
      // it is parsed as a key inside that section. We emit
      // developer_instructions first, then the section blocks.
      const configLines = [
      ...
      configLines.push(
        "",
        "# Suppress the 'under-development feature' warning Codex prints at",
        "# startup when codex_hooks is enabled. The hooks subsystem is",
        "# stable enough for capture telemetry; the warning is noise here.",
        "suppress_unstable_features_warning = true",
        "",
        "# Suppress the update prompt on startup. Codex still upgrades when",
        "# the user runs `npm install -g @openai/codex` manually; we just",
        "# skip the interactive nag every launch.",
        ...
        "network_access = true",
        "",
        "# Enable lifecycle hooks (UserPromptSubmit / PreToolUse /",
        "# PostToolUse / Stop / SessionStart / PermissionRequest). Required",
        "# for codi capture pipeline to populate brain.db with codex",
        "# sessions. The hooks live in .codex/hooks.json next to this file.",
        "[features]",
        "codex_hooks = true",
        ...(nativeSettings ? nativeSettings.filter((line) => line !== "[features]") : []),
      );
    ...
    const codiHook = (name: string): string =>
      `cd ${codexProjectRootRef} && codi hook ${name} --agent codex`;
    // Codex hooks schema (per https://developers.openai.com/codex/hooks):
    // root `hooks` object → event name array → matcher group with nested
    // `hooks` array. Matcher is a regex applied to tool_name; "*" or
    // omitted matches every tool. `codex_hooks = true` must be set in
    // config.toml for any of this to fire.
    const codexHooks = {
      hooks: {
        UserPromptSubmit: [
          {
            hooks: [
              {
                ...
                timeout: 10,
                statusMessage: "codi: capturing prompt",
              },
            ],
          },
        ],
            ...
            hooks: [
              {
                ...
                timeout: 30,
                statusMessage: "codi: pre-tool-use gates",
              },
            ],
          },
        ],
            ...
            hooks: [
              {
                ...
                timeout: 15,
                statusMessage: "codi: recording tool call",
              },
            ],
          },
        ],
        Stop: [
          {
            hooks: [
                ...
                : []),
              {
                ...
                timeout: 15,
                statusMessage: "codi: closing turn + tokens",
              },
            ],
          },
        ],
