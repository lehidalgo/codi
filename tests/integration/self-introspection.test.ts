...
beforeEach(async () => {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-self-'));
...
describe('Self-introspection (dogfooding)', () => {
  ...
  it('init rejects unknown agent IDs', async () => {
    const result = await initHandler(tmpDir, { agents: ['claude-code', 'unknown-agent'] });
  ...
  it('full pipeline: init, add rules, validate, generate, status, drift', { timeout: 15000 }, async () => {
    ...
    // 2. Add rules
    const codeQuality = await addRuleHandler(tmpDir, 'code-quality', { template: 'code-style' });
    expect(codeQuality.success).toBe(true);

    const security = await addRuleHandler(tmpDir, 'security', { template: 'security' });
    expect(security.success).toBe(true);

    const testing = await addRuleHandler(tmpDir, 'testing-standards', { template: 'testing' });
    ...
    const expectedFiles = ['CLAUDE.md', '.cursorrules', 'AGENTS.md', '.windsurfrules', '.clinerules'];
