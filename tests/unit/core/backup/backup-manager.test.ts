...
async function setup(): Promise<void> {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-backup-'));
  projectRoot = tmpDir;
...
async function writeState(filePaths: string[]): Promise<void> {
  ...
  await fs.writeFile(
    path.join(codiDir, STATE_FILENAME),
...
describe('createBackup', () => {
  it('creates a timestamped backup directory with files', async () => {
    ...
    // Act
    const timestamp = await createBackup(projectRoot, codiDir);
  ...
  it('copies file contents correctly', async () => {
    ...
    const backedUp = await fs.readFile(
      path.join(codiDir, BACKUPS_DIR, timestamp!, 'AGENTS.md'),
      'utf8',
    );
  ...
  it('returns null when state file does not exist', async () => {
    ...
    // Act
    const result = await createBackup(projectRoot, codiDir);
...
describe('listBackups', () => {
  it('returns backups sorted by timestamp newest first', async () => {
    ...
    // Act
    const result = await listBackups(codiDir);
...
describe('restoreBackup', () => {
  it('restores files to original locations', async () => {
    ...
    // Act
    const restored = await restoreBackup(projectRoot, codiDir, timestamp!);
  ...
  it('throws when backup does not exist', async () => {
    ...
    // Act & Assert
    await expect(
      restoreBackup(projectRoot, codiDir, 'nonexistent-timestamp'),
    ).rejects.toThrow();
  });
});

describe('backup directory structure', () => {
  it('stores backups under .codi/backups/<timestamp>/', async () => {
    // Arrange
    await setup();
    ...
    // Assert
    expect(timestamp).not.toBeNull();
    const expectedPath = path.join(codiDir, BACKUPS_DIR, timestamp!);
