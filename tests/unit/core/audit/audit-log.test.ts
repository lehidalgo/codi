...
async function setup(): Promise<void> {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-audit-'));
}
 ...
 */
async function readAuditLines(codiDir: string): Promise<AuditEntry[]> {
  const raw = await fs.readFile(path.join(codiDir, AUDIT_FILENAME), 'utf8');
  return raw
    .split('\n')
...
describe('writeAuditEntry', () => {
  ...
  it('preserves existing entries when appending', async () => {
    // Arrange
    await setup();
    const first = makeEntry({ type: 'init', timestamp: '2026-01-01T00:00:00.000Z' });
    const second = makeEntry({ type: 'generate', timestamp: '2026-01-02T00:00:00.000Z' });
...
describe('readAuditLog (file parsing)', () => {
  it('returns parsed entries from JSONL file', async () => {
    // Arrange
    await setup();
    await writeAuditEntry(tmpDir, makeEntry({ type: 'init', details: { preset: 'standard' } }));
    await writeAuditEntry(tmpDir, makeEntry({ type: 'generate', details: { files: 5 } }));
    await writeAuditEntry(tmpDir, makeEntry({ type: 'clean', details: { removed: 2 } }));
