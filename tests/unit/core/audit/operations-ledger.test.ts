...
beforeEach(async () => {
  tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-ledger-'));
...
describe('write', () => {
  ...
  it('creates parent directory if missing', async () => {
    const nestedDir = path.join(tmpDir, 'nested', 'deep', '.codi');
...
describe('setInitialization', () => {
  ...
  it('preserves existing operations when setting initialization', async () => {
    const existingOp = makeOperation({ type: 'generate', timestamp: '2026-01-01T01:00:00.000Z' });
...
describe('setActivePreset', () => {
  ...
  it('replaces previous active preset', async () => {
    ...
    const second = makePreset({ name: 'full', installedAt: '2026-01-02T00:00:00.000Z' });
...
describe('addGeneratedFiles', () => {
  it('adds new files to generated array', async () => {
    ...
    const file2 = makeGeneratedFile({ path: '.claude/rules/security.md', type: 'rule' });
...
describe('addHookFiles', () => {
  it('adds hook files to hooks array', async () => {
    ...
    const hook2 = makeHookFile({ path: '.husky/commit-msg', type: 'commit-msg' });
...
describe('addConfigFiles', () => {
  it('adds config files to config array', async () => {
    const cfg1 = makeConfigFile({ path: '.codi/state.json', type: 'state' });
    const cfg2 = makeConfigFile({ path: '.codi/manifest.json', type: 'manifest' });
  ...
  it('deduplicates by path', async () => {
    const cfg = makeConfigFile({ path: '.codi/state.json' });
...
describe('logOperation', () => {
  ...
  it('preserves chronological order', async () => {
    const op1 = makeOperation({ type: 'generate', timestamp: '2026-01-01T01:00:00.000Z' });
    const op2 = makeOperation({ type: 'clean', timestamp: '2026-01-01T02:00:00.000Z' });
    const op3 = makeOperation({ type: 'update', timestamp: '2026-01-01T03:00:00.000Z' });
