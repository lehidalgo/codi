...
import { validatePreset, detectCircularExtends } from '../../../../src/core/preset/preset-validator.js';

function makeTmpDir(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), 'codi-val-test-'));
}
...
describe('validatePreset', () => {
  ...
  it('valid preset passes validation', async () => {
    const manifest = { name: 'valid-preset', version: '1.0.0', description: 'A valid preset' };
  ...
  it('counts artifacts in subdirectories', async () => {
    ...
    await fs.writeFile(path.join(rulesDir, 'rule-one.md'), '---\ndescription: test\n---\nContent', 'utf-8');
    await fs.writeFile(path.join(rulesDir, 'rule-two.md'), '---\ndescription: test2\n---\nMore content', 'utf-8');
  ...
  it('fails when YAML is invalid', async () => {
    await fs.writeFile(path.join(tmpDir, 'preset.yaml'), '{{invalid yaml: [}', 'utf-8');
...
describe('detectCircularExtends', () => {
  ...
  it('detects direct circular reference', async () => {
    ...
    await fs.writeFile(path.join(presetA, 'preset.yaml'), yamlStringify({ name: 'a', extends: 'b' }), 'utf-8');
    await fs.writeFile(path.join(presetB, 'preset.yaml'), yamlStringify({ name: 'b', extends: 'a' }), 'utf-8');
  ...
  it('returns ok for valid extends chain', async () => {
    ...
    await fs.mkdir(presetA, { recursive: true });
    await fs.mkdir(presetB, { recursive: true });
    await fs.writeFile(path.join(presetA, 'preset.yaml'), yamlStringify({ name: 'child', extends: 'parent' }), 'utf-8');
    await fs.writeFile(path.join(presetB, 'preset.yaml'), yamlStringify({ name: 'parent' }), 'utf-8');
