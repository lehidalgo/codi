...
import { mkdir, writeFile, rm, readFile, readdir, access } from 'node:fs/promises';
...
describe('skill-export', () => {
  ...
  beforeEach(async () => {
    await mkdir(join(codiDir, 'skills', 'test-skill', 'scripts'), { recursive: true });
    await mkdir(join(codiDir, 'skills', 'test-skill', 'references'), { recursive: true });
    await mkdir(join(codiDir, 'skills', 'test-skill', 'assets'), { recursive: true });
    await mkdir(join(codiDir, 'skills', 'test-skill', 'evals'), { recursive: true });
    await writeFile(join(codiDir, 'skills', 'test-skill', 'SKILL.md'), VALID_SKILL_MD);
    await writeFile(join(codiDir, 'skills', 'test-skill', 'scripts', 'helper.sh'), '#!/bin/bash\necho "hello"');
    await writeFile(join(codiDir, 'skills', 'test-skill', 'references', 'guide.md'), '# Guide');
    await writeFile(join(codiDir, 'skills', 'test-skill', 'scripts', '.gitkeep'), '');
    await writeFile(join(codiDir, 'skills', 'test-skill', 'evals', 'evals.json'), '[]');
  ...
  describe('validateSkillForExport', () => {
    ...
    it('returns error when description is empty', async () => {
      ...
      await writeFile(join(noDescDir, 'SKILL.md'), '---\nname: no-desc\ndescription: ""\n---\nContent');
  ...
  describe('standard export', () => {
    ...
    it('excludes .gitkeep files', async () => {
      ...
      const scriptsFiles = await readdir(join(result.data.outputPath, 'scripts'));
  ...
  describe('claude-plugin export', () => {
    ...
    it('copies supporting files into plugin structure', async () => {
      ...
      const helper = await readFile(
        join(result.data.outputPath, 'skills', 'test-skill', 'scripts', 'helper.sh'),
        'utf-8',
      );
      expect(helper).toContain('echo "hello"');
    });
