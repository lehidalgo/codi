...
function makeFlags(overrides: Record<string, { value: unknown; mode: string }>): ResolvedFlags {
  const base: ResolvedFlags = {
    security_scan: { value: true, mode: 'enabled', source: 'default', locked: false },
    type_checking: { value: 'strict', mode: 'enabled', source: 'default', locked: false },
    max_file_lines: { value: 700, mode: 'enabled', source: 'default', locked: false },
  };
...
describe('generateHooksConfig', () => {
  ...
  it('excludes typecheck hooks when type_checking is off', () => {
    const flags = makeFlags({ type_checking: { value: 'off', mode: 'enabled' } });
  ...
  it('includes typecheck hooks when type_checking is strict', () => {
    const flags = makeFlags({ type_checking: { value: 'strict', mode: 'enabled' } });
    const config = generateHooksConfig(flags, ['typescript']);
    expect(config.hooks.map((h) => h.name)).toContain('tsc');
  });
  ...
  it('disables secret scan when security_scan flag is disabled', () => {
    const flags = makeFlags({ security_scan: { value: false, mode: 'disabled' } });
    const config = generateHooksConfig(flags, ['typescript']);
    expect(config.secretScan).toBe(false);
  });

  it('enables file size check when max_file_lines is set', () => {
    const config = generateHooksConfig(makeFlags({}), ['typescript']);
    expect(config.fileSizeCheck).toBe(true);
    expect(config.maxFileLines).toBe(700);
  });

  it('disables file size check when max_file_lines is disabled', () => {
    const flags = makeFlags({ max_file_lines: { value: 700, mode: 'disabled' } });
    const config = generateHooksConfig(flags, ['typescript']);
    expect(config.fileSizeCheck).toBe(false);
  });
