...
import { marketplaceSearchHandler, marketplaceInstallHandler } from '../../../src/cli/marketplace.js';
import { Logger } from '../../../src/core/output/logger.js';
import { EXIT_CODES } from '../../../src/core/output/exit-codes.js';
import { REGISTRY_INDEX_FILENAME } from '../../../src/constants.js';
...
beforeEach(async () => {
  ...
  mockRegistryDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-mock-registry-'));

  const registryIndex = [
    { name: 'test-skill', description: 'A test skill for unit testing', path: 'skills/test-skill.md' },
    { name: 'another-skill', description: 'Another skill for search', path: 'skills/another-skill.md' },
  ];
  await fs.writeFile(
  ...
  await fs.writeFile(path.join(skillsDir, 'another-skill.md'), '# Another Skill\nMore content.', 'utf-8');
  ...
  mockedExecFile.mockImplementation((...args: unknown[]) => {
    ...
    return { on: vi.fn(), stdout: null, stderr: null, pid: 0 } as unknown as ReturnType<typeof execFile>;
  });
});
...
describe('marketplace search handler', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-mp-search-'));
  ...
  it('searches and finds matching skills', async () => {
    ...
    expect(result.data.results!.some((r) => r.name === 'test-skill')).toBe(true);
    expect(result.exitCode).toBe(EXIT_CODES.SUCCESS);
  });

  it('returns empty results for non-matching query', async () => {
    const result = await marketplaceSearchHandler(tmpDir, 'nonexistent-xyz', {});
...
describe('marketplace install handler', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-mp-install-'));
    Logger.init({ level: 'error', mode: 'human', noColor: true });
  });
  ...
  it('installs an existing skill', async () => {
    ...
    const destPath = path.join(tmpDir, '.codi', 'skills', 'test-skill.md');
    const content = await fs.readFile(destPath, 'utf-8');
    expect(content).toContain('Test Skill');
  });

  it('fails for a non-existent skill', async () => {
    const result = await marketplaceInstallHandler(tmpDir, 'does-not-exist', {});
