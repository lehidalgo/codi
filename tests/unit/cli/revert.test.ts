...
import { BACKUPS_DIR, BACKUP_MANIFEST_FILENAME } from '../../../src/constants.js';
...
describe('revert command handler', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codi-revert-'));
  ...
  it('returns error when no option specified', async () => {
    ...
    expect(result.errors![0]!.message).toContain('Specify --list, --last, or --backup');
  });
