...
describe('flag presets', () => {
  it('has 3 preset names', () => {
    expect(getPresetNames()).toEqual(['minimal', 'balanced', 'strict']);
  });
  ...
  it('each preset flag has a valid mode', () => {
    const validModes = ['enforced', 'enabled', 'disabled', 'inherited', 'delegated_to_agent_default', 'conditional'];
    for (const name of getPresetNames()) {
      const preset = getPreset(name);
      for (const [flag, def] of Object.entries(preset)) {
