...
describe('claude-code adapter', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `codi-test-claude-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('generate() — rule files', () => {
    ...
    it('rule files have no YAML frontmatter', async () => {
      ...
      for (const rule of ruleFiles) {
        // Content after generated header should NOT contain frontmatter delimiters
        const afterHeader = rule.content.replace(/^<!-- Generated by Codi.*?-->\n\n/, '');
        expect(afterHeader).not.toMatch(/^---/);
      }
    });
    ...
    it('rule files contain their name as heading and content', async () => {
      ...
      expect(codeStyleRule.content).toContain('# Code Style');
  ...
  describe('generate() — command files', () => {
    ...
    it('command files contain frontmatter with description', async () => {
      ...
      expect(cmdFile.content).toContain('description: Deploy to production');
      ...
      const config = createMockConfig({
        mcp: {
          servers: {
            ...
            disabled: { type: 'stdio', command: 'npx', args: [], enabled: false },
          },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
  ...
  describe('generate() — settings.json (buildSettingsJson)', () => {
    it('produces .claude/settings.json when max_context_tokens flag is set', async () => {
      const config = createMockConfig({
        flags: {
          max_context_tokens: { value: 50_000, mode: 'enforced', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
    ...
    it('caps autocompact percentage at 70%', async () => {
      ...
      const config = createMockConfig({
        flags: {
          max_context_tokens: { value: 180_000, mode: 'enforced', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
      const settingsFile = files.find(f => f.path === '.claude/settings.json')!;
    ...
    it('caps at 70% even when max_context_tokens equals CONTEXT_TOKENS_LARGE', async () => {
      const config = createMockConfig({
        flags: {
          max_context_tokens: { value: CONTEXT_TOKENS_LARGE, mode: 'enforced', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
      const settingsFile = files.find(f => f.path === '.claude/settings.json')!;
    ...
    it('returns null (no settings.json) when no relevant flags are set', async () => {
      const config = createMockConfig({
        flags: {
          lint_on_save: { value: true, mode: 'enabled', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
    ...
    it('generates enabledMcpjsonServers for mcp_allowed_servers', async () => {
      const config = createMockConfig({
        flags: {
          mcp_allowed_servers: { value: ['memory', 'docs'], mode: 'enabled', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
      const settingsFile = files.find(f => f.path === '.claude/settings.json');
    ...
    it('returns null when max_context_tokens is zero', async () => {
      const config = createMockConfig({
        flags: {
          max_context_tokens: { value: 0, mode: 'enforced', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
    ...
    it('returns null when max_context_tokens is not a number', async () => {
      const config = createMockConfig({
        flags: {
          max_context_tokens: { value: 'auto', mode: 'enforced', source: 'codi.yaml', locked: false },
        },
      });
      const files = await claudeCodeAdapter.generate(config, {});
