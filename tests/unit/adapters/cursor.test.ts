...
describe('cursor adapter', () => {
  ...
  it('generates skill files in .cursor/skills/', async () => {
    const config = createMockConfig({
      skills: [
        { name: 'deploy', description: 'Deployment skill', content: 'Run deploy commands' },
        { name: 'review', description: 'Code review', content: 'Review code carefully' },
      ],
    });
    const files = await cursorAdapter.generate(config, {});
    ...
    const config = createMockConfig({
      mcp: {
        servers: {
          ...
          'disabled-server': { command: 'node', args: ['b.js'], enabled: false },
        },
      },
    });
    const files = await cursorAdapter.generate(config, {});
  ...
  it('produces different hashes for different configs', async () => {
    const config1 = createMockConfig({
      flags: {
        allow_shell_commands: { value: false, mode: 'enforced', source: 'codi.yaml', locked: false },
      },
    });
    const config2 = createMockConfig({
      flags: {
        allow_shell_commands: { value: true, mode: 'enforced', source: 'codi.yaml', locked: false },
        max_file_lines: { value: 1000, mode: 'enforced', source: 'codi.yaml', locked: false },
      },
    });
