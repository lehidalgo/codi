...
describe('generator', () => {
  const tmpDir = join(tmpdir(), 'codi-test-generator-' + Date.now());
  ...
  it('generates files for specified agents', async () => {
    const config = createMockConfig({ manifest: { name: 'test', version: '1', agents: ['claude-code'] } });
  ...
  it('writes files to disk', async () => {
    const config = createMockConfig({ manifest: { name: 'test', version: '1', agents: ['codex'] } });
    const result = await generate(config, tmpDir);
  ...
  it('skips file writing in dryRun mode', async () => {
    ...
    await expect(readFile(join(tmpDir, 'AGENTS.md'), 'utf-8')).rejects.toThrow();
  });

  it('returns error for unknown adapter', async () => {
    const config = createMockConfig({ manifest: { name: 'test', version: '1', agents: ['unknown-agent'] } });
    const result = await generate(config, tmpDir);
