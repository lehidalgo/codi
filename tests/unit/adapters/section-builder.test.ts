...
describe('buildProjectOverview', () => {
  ...
  it('includes description when present', () => {
    const config = createMockConfig({
      manifest: { name: 'my-proj', version: '1', agents: [], description: 'A cool project' },
    });
    const result = buildProjectOverview(config);
...
describe('buildDevelopmentNotes', () => {
  ...
  it('includes notes for enabled flags', () => {
    const config = createMockConfig({
      flags: {
        test_before_commit: { value: true, mode: 'enforced', source: 'codi.yaml', locked: false },
        security_scan: { value: true, mode: 'enforced', source: 'codi.yaml', locked: false },
        type_checking: { value: 'strict', mode: 'enforced', source: 'codi.yaml', locked: false },
        require_pr_review: { value: true, mode: 'enforced', source: 'codi.yaml', locked: false },
        allow_force_push: { value: false, mode: 'enforced', source: 'codi.yaml', locked: false },
        lint_on_save: { value: true, mode: 'enforced', source: 'codi.yaml', locked: false },
      },
    });
