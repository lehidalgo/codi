...
describe("MCP server templates", () => {
  it("AVAILABLE_MCP_SERVER_TEMPLATES matches BUILTIN_MCP_SERVERS keys", () => {
    expect(AVAILABLE_MCP_SERVER_TEMPLATES).toEqual(
      Object.keys(BUILTIN_MCP_SERVERS),
    );
  });
  ...
  it("all templates have either command or url", () => {
    for (const [key, tmpl] of Object.entries(BUILTIN_MCP_SERVERS)) {
      ...
      expect(
        hasCommand || hasUrl,
        `Template "${key}" must have command or url`,
      ).toBe(true);
    }
  });
  ...
  it("stdio templates have args array", () => {
    for (const [key, tmpl] of Object.entries(BUILTIN_MCP_SERVERS)) {
      if (tmpl.command) {
        expect(
          Array.isArray(tmpl.args),
          `Template "${key}" with command should have args`,
        ).toBe(true);
      }
    }
  });
